...
public class ServicesTest extends AbstractTest {
	...
	// Test---------------------------------------------------------------
	// cada vez que se realiza la prueba se ha de ejecutar el populate para que se
	// borren las referencias y objetos creados en los test que se han ejecutado
	// anteriormente sino da error al intentar referenciar varias veces el mismo
	// objeto de la BD
	@Test
	public void servicesTest() {
		final Object testingData[][] = {
				...
				// 2. Request a service for one of the rendezvouses that he or she is created.
				// He or she
				// must specify a valid credit card in every request for a service.

				// An actor who is registered as a manager must be able to:
				// 1. List the services that are available in the system.
				// 2. Manage his or her services, which includes listing them, creating them,
				// updating
				// them, and deleting them as long as they are not required by any rendezvouses.

				// An actor who is authenticated as an administrator must be able to:
				// Cancel a service that he or she finds inappropriate. Such services cannot be
				// requested
						...
						null, IllegalArgumentException.class },
				{
						...
						"manager1", "name1", "description1", false, 1, "rendezvous3", null },
				{
						// User creates a service.
						"user1", "name2", "description2", false, 1, "rendezvous3", IllegalArgumentException.class },
				{
		...
		for (int i = 3; i < 5; i++)
			this.createTemplate((String) testingData[i][0], (String) testingData[i][1], (String) testingData[i][2],
					(boolean) testingData[i][3], (int) testingData[i][4], (String) testingData[i][5],
					(Class<?>) testingData[i][6]);

		for (int i = 5; i < 8; i++)
			this.editTemplate((String) testingData[i][0], (String) testingData[i][1], (String) testingData[i][2],
	...
	protected void createTemplate(final String user, final String name, final String description,
			final boolean canceled, final int level, final String rendezvous, final Class<?> expected) {
		Class<?> caught;
		caught = null;
		try {
